...
typedef testing::TestWithParam<std::tuple<size_t, size_t>> TestHashMapSuite0;
TEST_P(TestHashMapSuite0, insert) {
...
INSTANTIATE_TEST_SUITE_P(/**/, TestHashMapSuite0, 
    testing::Combine(
...
typedef testing::TestWithParam<std::tuple<size_t, size_t>> TestHashMapSuite1;
TEST_P(TestHashMapSuite1, erase) {
...
INSTANTIATE_TEST_SUITE_P(/**/, TestHashMapSuite1, 
    testing::Combine(
        testing::ValuesIn(threads_count_suite0),
        testing::ValuesIn(elem_count)
    )
...
typedef testing::TestWithParam<std::tuple<size_t, size_t>> TestHashMapSuite2;
TEST_P(TestHashMapSuite2, read) {
...
INSTANTIATE_TEST_SUITE_P(/**/, TestHashMapSuite2, 
    testing::Combine(
        testing::ValuesIn(threads_count_suite0),
        testing::ValuesIn(elem_count)
    )
...
typedef testing::TestWithParam<std::tuple<size_t, size_t>> TestHashMapSuite3;
TEST_P(TestHashMapSuite3, common) {
    auto func_read = [](HashMap<int, int>& m, int& res, size_t count, size_t mapSize) {
        int sum = 0;
        std::mt19937 rnd;
        for (int i = 0; i < count; i++) {
    ...
    sort(times.begin(), times.end());
    double mean_time = std::accumulate(times.begin(), times.end() - Storage::TRUNCATIONS, 0.0) / (times.size() - Storage::TRUNCATIONS);
    Storage::testRes[0][3][threads_num][count_per_thread] = mean_time;
}


INSTANTIATE_TEST_SUITE_P(/**/, TestHashMapSuite3, 
    testing::Combine(
