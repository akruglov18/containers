...
void pushBacks(ParallelVector<int>& v, int s, int e) {
    for (int i = s; i < e; i++) {
        v.push_back(i);
    }
}

typedef testing::TestWithParam<std::tuple<size_t, size_t>> TestParallelVector;
TEST_P(TestParallelVector, push_back) {
...
INSTANTIATE_TEST_SUITE_P(/**/, TestParallelVector, 
    testing::Combine(
