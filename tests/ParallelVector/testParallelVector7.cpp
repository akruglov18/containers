...
typedef testing::TestWithParam<std::tuple<size_t, size_t>> TestParallelVector7Suite0;
TEST_P(TestParallelVector7Suite0, push_back) {
    auto params = GetParam();
    const size_t threads_num = std::get<0>(params);
    const size_t count_per_thread = std::get<1>(params);
    std::vector<double> times;
    auto func_push_back = [](ParallelVector7<int, 50000>& v, size_t count) {
        for (int i = 0; i < count; i++) {
            v.push_back(i);
        }
    };
    for (int t = 0; t < Storage::RUNS; t++) {
        ParallelVector7<int, 50000> v;
    ...
    Storage::testRes[6][0][threads_num][count_per_thread] = mean_time;
}

INSTANTIATE_TEST_SUITE_P(/**/, TestParallelVector7Suite0, 
    testing::Combine(
...
typedef testing::TestWithParam<std::tuple<size_t, size_t>> TestParallelVector7Suite1;
TEST_P(TestParallelVector7Suite1, read) {
    auto params = GetParam();
    const size_t threads_num = std::get<0>(params);
    const size_t count_per_thread = std::get<1>(params);
    std::vector<double> times;
    for (int t = 0; t < Storage::RUNS; t++) {
        ...
        auto func = [](ParallelVector7<int, 50000>& v, std::vector<int>& res, int num, size_t count) {
            int sum = 0;
            for (int i = 0; i < count; i++) {
                if (i < v.get_size())
                {
    ...
    sort(times.begin(), times.end());
    double mean_time = std::accumulate(times.begin(), times.end() - Storage::TRUNCATIONS, 0.0) / (times.size() - Storage::TRUNCATIONS);
    Storage::testRes[6][1][threads_num][count_per_thread] = mean_time;
}

INSTANTIATE_TEST_SUITE_P(/**/, TestParallelVector7Suite1, 
    testing::Combine(
...
typedef testing::TestWithParam<std::tuple<size_t, size_t>> TestParallelVector7Suite2;
TEST_P(TestParallelVector7Suite2, common_test) {
    auto func_push_back = [](ParallelVector7<int, 50000>& v, size_t count) {
        for (int i = 0; i < count; i++) {
            v.push_back(i);
        }
    };
    auto func_read = [](ParallelVector7<int, 50000>& v, int& res, size_t count) {
        int sum = 0;
        std::mt19937 rnd;
        for (int i = 0; i < count; i++) {
            if (v.get_size() > 0) {
    ...
    auto func_write = [](ParallelVector7<int, 50000>& v, size_t count) {
        std::mt19937 rnd;
        for (int i = 0; i < count; i++) {
            if (v.get_size() > 0) {
                int ind = rnd() % v.get_size();
    ...
    sort(times.begin(), times.end());
    double mean_time = std::accumulate(times.begin(), times.end() - Storage::TRUNCATIONS, 0.0) / (times.size() - Storage::TRUNCATIONS);
    Storage::testRes[6][2][threads_num][count_per_thread] = mean_time;
}

INSTANTIATE_TEST_SUITE_P(/**/, TestParallelVector7Suite2, 
    testing::Combine(
