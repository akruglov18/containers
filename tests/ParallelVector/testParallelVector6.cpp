...
typedef testing::TestWithParam<std::tuple<size_t, size_t>> TestParallelVector6Suite0;
TEST_P(TestParallelVector6Suite0, push_back) {
...
typedef testing::TestWithParam<std::tuple<size_t, size_t>> TestParallelVector6Suite1;
TEST_P(TestParallelVector6Suite1, read) {
    auto params = GetParam();
    const size_t threads_num = std::get<0>(params);
    const size_t count_per_thread = std::get<1>(params);
    // std::vector<double> times;
...
typedef testing::TestWithParam<std::tuple<size_t, size_t>> TestParallelVector6Suite2;
TEST_P(TestParallelVector6Suite2, common_test) {
    auto func_push_back = [](ParallelVector6<int>& v, size_t count) {
        for (int i = 0; i < count; i++) {
            v.push_back(i);
        }
    };
    auto func_read = [](ParallelVector6<int>& v, int& res, size_t count) {
        int sum = 0;
        std::mt19937 rnd;
        for (int i = 0; i < count; i++) {
            if (v.get_size() > 0) {
    ...
    auto func_write = [](ParallelVector6<int>& v, size_t count) {
        std::mt19937 rnd;
        for (int i = 0; i < count; i++) {
            if (v.get_size() > 0) {
                int ind = rnd() % v.get_size();
